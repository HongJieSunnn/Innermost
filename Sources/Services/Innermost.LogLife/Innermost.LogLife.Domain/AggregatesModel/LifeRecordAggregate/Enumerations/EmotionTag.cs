...
namespace Innermost.LogLife.Domain.AggregatesModel.LifeRecordAggregate
{
    public class EmotionTag
        : Enumeration
    {
        public static IEnumerable<EmotionTag> EmotionTagEnumerable => GetEmotionTags(); //TODO 若心情标签有多种且可以不断增加，那么可能需要用到Redis存储并每次读取
        ...
        {
            var emotionTag = EmotionTagEnumerable.FirstOrDefault(n => n.Name.Equals(name));
            if (emotionTag == null)
        ...
        {
            var emotionTag = EmotionTagEnumerable.FirstOrDefault(n => n.Id.Equals(id));
            if (emotionTag == null)
